...
void Smoother::smoothPath(DynamicVoronoi& voronoi) {
  ...
  // 迭代优化 迭代次数 < 500
  // 这里没有提前 跳出的机制 非常不理想 如果优化目标差不多了 可以直接选择跳出的
  // 没必要为了最后一丝精度 浪费计算时间
  while (iterations < maxIterations) {
    ...
    for (int i = 2; i < pathLength - 2; ++i) {
      ...
      // 多个代价之后 累积最终用于修正的offset
      Vector2D correction;


      
      // 判断是否为尖点 尖点不需要平滑 直接跳过
      // 因为尖点 不可导 （尖点 指的是前进 马上后退或者 后退 马上前进的那种情况）
      if (isCusp(newPath, i)) { 
          continue; }
      
      // 障碍物修正
      //                          obstacleTerm（）函数返回的负数 梯度的负方向
      correction = correction - obstacleTerm(xi);
      // 如果修正之后 不在地图上了 直接放弃 不进行修正 进行下一个点
      if (!isOnGrid(xi + correction)) { continue; }

      //todo not implemented yet
      // voronoi代价暂时没有做  所以该总权重代价也设为了 0
      // voronoiTerm(); 

      // 平滑修正 必须保证修正后的点 要在地图范围内
      // 这个平滑算法有意思  五点连成的图形 凸了就将中心点xi 往下拉
      //                               凹了就将中心点xi 往上拽
      correction = correction - smoothnessTerm(xim2, xim1, xi, xip1, xip2);
      if (!isOnGrid(xi + correction)) { continue; }

      // 曲率修正 必须保证修正后的点 要在地图范围内
      // 这个内部优化方法点 暂时没看懂
...
Vector2D Smoother::obstacleTerm(Vector2D xi) {
  ...
  // 当前点 在 voronoi中 距离障碍物最近的距离
  // ？？？ 对voronoi 图 还没了解 涉及
  // 暂时 这里将 obsDst 理解成点 xi 距离最近的障碍物位置
  ...
  if (x < width && x >= 0 && y < height && y >= 0) {
    ...
    if (obsDst < obsDMax) {
      // 计算梯度反方向 
      // obsVct / obsDst 理解成 障碍物 指向 xi的单位方向向量
      // (obsDst - obsDMax) 理解成真实距离的权重系数配比
      // wObstacle 理解成障碍物代价
...
//###################################################
// 曲率代价
Vector2D Smoother::curvatureTerm(Vector2D x_im2,  // 参数为依次五个点的坐标
                                 Vector2D x_im1, 
                                 Vector2D x_i, 
                                 Vector2D x_ip1, 
                                 Vector2D x_ip2) {
  ...
  // 计算相邻点之间的向量
  // . ---> . ---> . ---> . ---> . 
  // delta_x_im1  delta_x_i  delta_x_ip1  delta_x_ip2
  ...
  // 确保向量大小不能为零 也就是两点之间 不能重合
  if (delta_x_im1.length() > 0 && 
      delta_x_i.length() > 0 && 
      delta_x_ip1.length() > 0 && 
      delta_x_ip2.length() > 0) {

    // the angular change at the node
    // 近似的一种算法 
    // 根据三点 两个向量 计算曲率
    auto compute_kappa = [](const Vector2D& delta_x_0,  // 向量a
                            const Vector2D& delta_x_1,  // 向量b
                            float& delta_phi,           // 向量 a b 之间的夹角
                            // 三点间的曲率
                            float& kappa) {
        // std::acos（）计算反余弦  
       // Helper::clamp(n,lower,upper) 将n 限制在 [lower,upper] 内 
        //   a b 一条直线的时候    delta_phi == 0
        //                                      a * b / |a| * |b|    -1    1        
        delta_phi = std::acos(Helper::clamp(delta_x_0.dot(delta_x_1) / (delta_x_0.length() * delta_x_1.length()), -1, 1));
        /*
        先保证 a 不变 角度发生变化  b绕起点旋转  
         夹角 thate 变大 kappa值也变大 车辆转弯难度越来越大 
              a    b
            ----->------>   kappa == 0

        然后保证 夹角不变 当a变长 意味着更容易转向 kappa变小
        */ 
    ...
    // 当前点的曲率小于最大曲率 那么就不需要修改
    if (kappa_i <= kappaMax) {
      Vector2D zeros;
      return zeros;
    } 
    // 由于曲率过大 也就是最小转弯半径过小 车辆执行机构执行不了 需要调整
    // 下面为核心调整策略
    else {

      // 计算    -1 / [cos(theta) *cos(theta)]
      //    =   -1 /  sin(theta)
      ...
      // 依次计算kappa的倒数 d_kappa_im1 = dkappa_i / dxi 
      const float& d_delta_phi_im1 = compute_d_delta_phi(delta_phi_im1);
      const Vector2D& d_cos_delta_phi_im1 = delta_x_im1.ort(delta_x_i) / 
                                           (delta_x_im1.length() * delta_x_i.length());
      const Vector2D& d_kappa_im1 = 1. / 
                            delta_x_im1.length() * d_delta_phi_im1 * d_cos_delta_phi_im1;

      // 利用 d_kappa 配合权重  求出下降梯度
...
//###################################################
// 平滑代价 计算函数
Vector2D Smoother::smoothnessTerm(Vector2D xim2, // 五个点的坐标值    a
                                  Vector2D xim1,  //                b
                                  Vector2D xi,          //          c
                                  Vector2D xip1,   //               d
                                  //                                e
                                  Vector2D xip2) {
  // 为啥要这要计算
  // 这里注意 这里是向量之间的运算 都是重载后的vector重载的运算符
      // 这个平滑算法有意思  五点连成的图形 凸了就将中心点xi 往下拉
      //                               凹了就将中心点xi 往上拽
      // 由算法是一个个点 往后优化的 所以最后就是为了尽量消除“尖点”的存在
  /*
           .          类似这种转换
         .   .        ------>      .  .  .
      .        .                .          .
  ...
  */
 //令 alpha = a - b  beta = 3(c - b)  gama = 3(c-d) yeta = e -d
 // 可将该公式拆解成   （alpha + yeta）+ （beta + gama）
 // 
